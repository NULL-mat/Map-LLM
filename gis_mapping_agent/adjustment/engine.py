...
from ..utils.geo_io import read_vector_file
...
class ModificationEngine:
    ...
    def _add_layer(self, map_state: MapState, layer_name: str, parameters: Dict[str, Any]) -> None:
        ...
        style = dict(parameters.get("style") or {})
        style_aliases = {
            "line_width": "linewidth",
            "line_style": "linestyle",
            "edge_color": "edgecolor",
        ...
        style_fields = set(style_aliases) | {
            "color", "linewidth", "linestyle", "edgecolor", "facecolor",
            "marker", "size", "alpha", "hatch", "attribute_column", "label_column",
        }
        for key, value in parameters.items():
            normalized_key = style_aliases.get(key, key)
            if normalized_key in style_fields and value is not None:
        ...
        # 读取数据以获取几何类型
        gdf = read_vector_file(data_path)
