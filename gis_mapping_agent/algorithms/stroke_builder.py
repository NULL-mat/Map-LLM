...
def _line_coordinates(geom):
    mapping = getattr(geom, "__geo_interface__", None)
    if not mapping:
        return []

    geom_type = mapping.get("type")
    coordinates = mapping.get("coordinates") or []
    if geom_type == "LineString":
        return [tuple(coord[:2]) for coord in coordinates if coord and len(coord) >= 2]
    if geom_type == "MultiLineString" and coordinates:
        first_line = coordinates[0] or []
        return [tuple(coord[:2]) for coord in first_line if coord and len(coord) >= 2]
    return []
...
class StrokeBuilder:
    ...
    def _calculate_deflection_angle(
        ...
        """
        # 获取线段方向
        coords1 = _line_coordinates(line1)
        coords2 = _line_coordinates(line2)
