...
def _plot_geometry_collection(ax, gdf: gpd.GeoDataFrame, color: str, linewidth: float, alpha: float) -> None:
    """Plot geometries without GeoPandas/Shapely vectorized plotting paths."""
    line_segments = []
    polygon_segments = []
    for geom in gdf.geometry:
        if _is_empty_geometry(geom):
            continue
        _collect_geometry_segments(geom, line_segments, polygon_segments)
...
def _plot_single_geometry(ax, geom, color: str, linewidth: float, alpha: float) -> None:
    line_segments = []
    polygon_segments = []
    _collect_geometry_segments(geom, line_segments, polygon_segments)
    if line_segments:
        ax.add_collection(LineCollection(line_segments, colors=color, linewidths=linewidth, alpha=alpha))
    if polygon_segments:
        ax.add_collection(PolyCollection(polygon_segments, facecolors=color, edgecolors=color, linewidths=linewidth, alpha=alpha))


def _is_empty_geometry(geom) -> bool:
    if geom is None:
        return True
    try:
...
def _collect_geometry_segments(geom, line_segments: list, polygon_segments: list) -> None:
    mapping = getattr(geom, "__geo_interface__", None)
    if not mapping:
        return

    geom_type = mapping.get("type") or getattr(geom, "geom_type", "")
    coordinates = mapping.get("coordinates")

    if geom_type == "LineString":
        segment = _xy_segment(coordinates)
        if segment:
    ...
    if geom_type == "Polygon":
        ...
        if rings:
            exterior = _xy_segment(rings[0])
            if exterior:
                polygon_segments.append(exterior)
        return
    ...
    if geom_type == "GeometryCollection":
        for part in geom.geoms:
            _collect_geometry_segments(part, line_segments, polygon_segments)


def _xy_segment(coordinates) -> list:
    if not coordinates:
        return []
    ...
    for coord in coordinates:
        if coord is None or len(coord) < 2:
            continue
        segment.append((float(coord[0]), float(coord[1])))
    return segment
...
class RoadNetworkGeneralizationEngine:
    ...
    def visualize_comparison(
        ...
        # 综合前
        _plot_geometry_collection(ax1, input_gdf, color='blue', linewidth=0.5, alpha=0.7)
        if algorithm == 'gcnn':
        ...
        # 综合后
        _plot_geometry_collection(ax2, output_gdf, color='red', linewidth=0.5, alpha=0.7)
