...
class MapOperationsMixin:
    ...
    def add_layer(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 使用绝对路径读取文件
            gdf = read_vector_file(str(file_path))
    ...
    def style_layer(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for item in self.current_map_state.legend_items:
                item_layer_name = (item.style or {}).get("layer_name") or (item.style or {}).get("_layer_name")
                if item.label == chinese_name or item_layer_name == layer_name:
                    item.style = new_style.model_dump()
                    item.style["layer_name"] = layer_name
