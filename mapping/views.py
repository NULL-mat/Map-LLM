...
def showcase(request):
    """Public project overview for the intelligent mapping workspace."""
    return render(request, 'mapping/showcase.html')
...
def process_map_request(request):
    ...
    try:
        ...
        worker = threading.Thread(
            target=_process_map_request_in_background,
            args=(map_request.id,),
            daemon=True,
            name=f"map-request-{map_request.id}",
        )
        worker.start()
...
def _process_map_request_in_background(map_request_id):
    """在线程中执行制图任务，让前端可以实时轮询中间结果。"""
    close_old_connections()
    try:
        map_request = MapRequest.objects.get(id=map_request_id)
        with MAP_LLM_EXECUTION_LOCK:
            _process_with_map_llm(map_request)
    except Exception as e:
        try:
            map_request = MapRequest.objects.get(id=map_request_id)
...
def continue_conversation(request):
    ...
    try:
        ...
        worker = threading.Thread(
            ...
            args=(map_request.id, message_text),
            daemon=True,
            name=f"map-request-continue-{map_request.id}",
        )
        worker.start()
...
def _continue_conversation_in_background(map_request_id, message_text):
    ...
    try:
        ...
        with MAP_LLM_EXECUTION_LOCK:
            try:
                ...
                response = agent.chat(message_text, session_id=session_id)
            finally:
                try:
            ...
            if image_files:
                ...
                try:
                    ...
                    if state is not None:
                        state_version = state.get_current_version()
                except Exception:
                    pass
        ...
        for map_request in requests:
            ...
            for map_obj in map_request.generated_maps.order_by("-created_at"):
                ...
                maps_data.append({
                    ...
                    "filename": map_obj.filename,
                    "version": map_obj.version,
                    "state_version": map_obj.state_version,
...
def activate_history_map(request):
    ...
    try:
        ...
        try:
            ...
            state_version = getattr(generated_map, "state_version", None)
            if not state_version:
                # Legacy records predate GeneratedMap.state_version. Their image
                # number and MapState version can diverge after branch activation,
                # so match the rendered output name before using the old fallback.
                expected_name = re.sub(r"^v\d+_", "", generated_map.filename)
                for version_info in state_manager.list_versions(session_id):
                    candidate_version = version_info.get("version")
                    candidate_state = state_manager.load_state(session_id, candidate_version)
                    output_name = Path(candidate_state.output_path or "").name if candidate_state else ""
                    if output_name == expected_name:
                        state_version = candidate_version
                        generated_map.state_version = state_version
                        generated_map.save(update_fields=["state_version"])
                        break

            state_version = state_version or generated_map.version
            map_state = state_manager.load_state(session_id, state_version)
            if not map_state:
                return JsonResponse({
                    "success": True,
                    "active_state": False,
            ...
            if not state_manager.save_state(map_state):
                raise RuntimeError("无法保存已激活的历史地图状态")

            # A process-level SessionContext can still contain the map that was
            # active before this switch. Clear it so the next message hydrates
            # the newly selected current version from persistent storage.
            from gis_mapping_agent.state import clear_session_context
            clear_session_context(session_id)

            cloned = active_version != state_version
            return JsonResponse({
                "success": True,
                "active_state": True,
                "request_id": map_request.id,
